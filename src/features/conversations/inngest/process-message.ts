...
export const processMessage = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      ...
      // Updating the message with error content
      if (internalKey) {
  ...
  async ({ event, step }) => {
    ...
    await step.sleep("wait-for-ai-processing", "2s");

    //Throwing an error to test
    /*await step.run("throw-on-purpose", () => {
      throw new NonRetriableError("Purposely throwing an error");
    });*/
